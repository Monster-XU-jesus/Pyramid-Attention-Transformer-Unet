...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        print(f"patch_size: {patch_size}, embed_dim: {embed_dim}")
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            # 在每个阶段都会使用不同的 position embedding
            # 每个stage的pos_embed是用来调节每个阶段输入的不同位置？
            pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dims[i]))
            pos_drop = nn.Dropout(p=drop_rate)

            # 上面是金字塔层数，那这里depths的作用是？depths才是决定block执行次数的参数
    ...
    def get_stage_features(self, stage_num):
        return self.stage_features[f"stage_{stage_num}"]
